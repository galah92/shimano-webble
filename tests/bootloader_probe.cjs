...
(async()=>{
 ...
 assert.equal(ok.calls.length,38);assert.deepEqual(Array.from(ok.result.mBootloaderVersion),[2,0,3,0]);
 assert.deepEqual(ok.stages,['baseline','M-update-entry','M-bootloader-version','D-update-entry','D-bootloader-entry','D-bootloader-identity','reset-request']);
