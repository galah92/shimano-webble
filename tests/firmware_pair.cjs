...
function fixture(fail){
 ...
 const options={files:[file(mi),file(di)],identityReference:identityReference(),baseline:{identity:'a'.repeat(64),family:34,unit:0,dVersion:'4.5.0.0',mVersion:'4.4.8.0',destination:0},credentials:keys,
  deviceId:'bike-a',journalStorage:storage(),characteristics:{'2afa':native,'2afe':native},subscribe(){throw Error('Controlled workers do not subscribe');},signal:controller.signal,
 ...
 ctx.readFirmwareBaseline=async()=>{record('baseline');return {identity:'a'.repeat(64),family:34,unit:0,dVersion:'4.5.0.0',mVersion:'4.4.8.0',destination:0};};
...
(async()=>{
 ...
 assert.deepEqual(f.calls,['baseline','entry1','slot','M','entry2','Dentry','D']);
 ...
 assert.equal(f.ctx.readFirmwareRecoveryJournal(f.options.journalStorage).stage,'paired-transfer-finished');
 for(const fail of ['baseline','entry1','slot','M','entry2','Dentry','D']){
 ...
 for(const change of [o=>o.files=[file(d),file(d)],o=>o.files=[file(d),file(new Uint8Array(256))],o=>o.baseline.family=35,o=>delete o.baseline.destination,o=>delete o.identityReference,o=>o.identityReference.verified=false,o=>delete o.identityReference.bootloaderIdentity,o=>o.identityReference.expected.identity='c'.repeat(64),o=>o.identityReference.salt='bad',o=>o.credentials=new Uint8Array(14),o=>delete o.characteristics['2afe'],o=>delete o.deviceId,o=>delete o.journalStorage]){
 ...
 f=fixture();f.options.journalStorage.setItem=()=>{throw Error('quota')};
 await assert.rejects(f.ctx.transferFirmwarePair(f.options));assert.deepEqual(f.calls,['baseline']);assert.equal(f.writes.length,0);
 for(const change of [{identity:'c'.repeat(64)},{dVersion:'4.3.0.0'},{mVersion:'4.2.1.0'},{destination:1},{family:35},{unit:1}]){
  f=fixture();const read=f.ctx.readFirmwareBaseline;f.ctx.readFirmwareBaseline=async()=>({...await read(),...change});
  await assert.rejects(f.ctx.transferFirmwarePair(f.options));assert.deepEqual(f.calls,['baseline']);assert.equal(f.writes.length,0);
 }
 f=fixture();const priorStage=f.options.onStage;f.options.onStage=stage=>{if(stage==='file-validation')f.options.baseline.destination=1;priorStage(stage);};
 await f.ctx.transferFirmwarePair(f.options);
 ...
 const real=vm.createContext({Uint8Array,TextEncoder,crypto:webcrypto});vm.runInContext(rawCode,real);
