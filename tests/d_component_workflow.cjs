...
async function run({family=34,unit=0,fail=-1,reject=-1,invalid=false}={}){
 ...
 const original=data.slice(),ctx=vm.createContext({Uint8Array,Error,setTimeout(fn,ms){const key=++id;timers.set(key,{fn,at:now+ms});return key;},clearTimeout(key){timers.delete(key);}});
 vm.runInContext(code,ctx);
 const replies={46:[58],47:[59],48:[60,family,unit],41:[53,9,8,7],42:[54,6,5,4]};
 ctx.transferDComponent({data,subscribe(fn){assert.equal(listener,null);listener=fn;return()=>listener=null;},
  async write(packet){
 ...
 for(let i=0;i<50&&!result&&!error;i++){
