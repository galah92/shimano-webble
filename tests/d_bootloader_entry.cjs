...
async function run({mode=0,selector=13,fail=-1,silent=-1,badStage=false,abortAt=-1}={}){
 let now=0,id=0,listener,result,error,selected=31,bridgeMode=mode;
 const timers=new Map(),calls=[],credentials=Uint8Array.from({length:15},(_,i)=>i+1),controller=new AbortController();
 ...
 const writer=ctx.createFirmwareGattWriter({'2afa':{async writeValueWithResponse(bytes){
  ...
  if(p[1]===6){selected=p[2];listener(Uint8Array.of(38,0));}
  if(p[1]===136&&listener)listener(Uint8Array.of(0,136,17,badStage?99:p[2]));
 }}},controller.signal);
 const pending=ctx.enterDBootloader({mode,targetSelector:selector,credentials,signal:controller.signal,write:writer.write,
  subscribe(fn){assert(!listener);listener=fn;return()=>listener=null;}}).then(r=>result=r,e=>error=e);
 ...
 return {ctx,calls,result,error,now};
}
(async()=>{
