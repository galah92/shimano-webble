...
async function run({mode=0,selector=13,fail=-1,silent=-1,badStage=false,rejected=false,abortAt=-1,recoveryInstall=false,slotStatus=0,stage1Timeouts=0}={}){
 let now=0,id=0,listener=null,result,error,selected=31,bridgeMode=mode,stage1Attempts=0;
 ...
 const writer=ctx.createFirmwareGattWriter({'2afa':{async writeValueWithResponse(bytes){
  ...
  if(p[1]===136&&p[2]===1&&recoveryInstall&&++stage1Attempts<=stage1Timeouts)return;
...
(async()=>{
 ...
 for(const [options,reason] of [
  ...
  [{stage1Timeouts:4},'reply-timeout'],
  [{fail:3},'transport-or-local-error'],
  [{abortAt:3},'disconnected'],
 ]) {
  const r=await run({mode:32,selector:0,recoveryInstall:true,...options});
  assert(r.error);assert.equal(r.calls.length,options.stage1Timeouts?7:4);
  assert.equal(r.diagnostics.at(-1).reason,reason);
  assert(!JSON.stringify(r.diagnostics).includes('native failure'));
 }
 for(const stage1Timeouts of [1,2,3]) {
  const r=await run({mode:32,selector:0,recoveryInstall:true,stage1Timeouts});assert(!r.error);
  assert.equal(r.calls.length,9+stage1Timeouts);assert.equal(r.now,1250+2000*stage1Timeouts);
  const attempts=r.calls.filter(c=>c.p[1]===136&&c.p[2]===1);
  assert.equal(attempts.length,stage1Timeouts+1);
  for(const a of attempts)assert.deepEqual(a.p,attempts[0].p);
  assert.equal(r.diagnostics.filter(e=>e.event==='retry-same-stage').length,stage1Timeouts);
 }
 for(const kind of ['fail','abortAt']) {
  const r=await run({mode:32,selector:0,recoveryInstall:true,stage1Timeouts:1,[kind]:4});
  assert(r.error);assert.equal(r.calls.length,5);
  assert.equal(r.diagnostics.filter(e=>e.event==='retry-same-stage').length,1);
 }
