...
(async()=>{
 ...
 for(const command of [[10,0,0,0],[36,0,64,0],[33,0,64,0],[39,69,0,0],[6,34,0,0],[7,9,8,0],[8,7,6,0],[9,5,4,0]]) {
  ...
  const deadline=command[0]===33?40000:[6,7,8,9,39].includes(command[0])?3000:1200;await f.advance(deadline-1);assert.equal(f.error,undefined);await f.advance(1);assert.match(f.error.message,/reply timed out/);assert.equal(f.writes.length,1);assert.equal(f.timers,0);
 }
 ...
 for(const payload of [[6,35,0,0],[6,34,1,0],[7,0,0,1],[8,0,0,1],[9,0,0,1],[39,0,1,0],[40,0,0,0],[0,0,0,0],[10,3,0,0],[33,0,96,0],[36,1,64,0],[36,0,0,0],[10,0,0,256]])assert.throws(()=>f.ctx.sendDFirmwareCommand({payload}),/Unsupported|range|unsigned/);
